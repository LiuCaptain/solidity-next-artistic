...
const environmentList = currentContract.environmentList
const environmentValue = await select({
	message: "请选择要部署的网络",
	choices: environmentList
})
const currentEnvironment = environmentList.find((item) => item.value === environmentValue)!
const chainId = currentEnvironment.chainId

const command = `${contractValue} ${environmentValue}`
